...
void changeState(FeedbackType type, int val){
    if(val <= 0){
        return;
    }
    switch (type)
    ...
    case CPUtemp: {
        ...
        int *Cputhres = returnPolicy(int(CPUtemp));
        if(Cputhres == nullptr){
            std::cout<<"null 포인터\n";
        }
        if(val < Cputhres[0]){
    ...
    std::cout<<int(DIMMtemp)<<"\n";
        int *Dimmthres = returnPolicy(int(DIMMtemp));
        if(val < Dimmthres[0]){
            foflManager.dimmTempFOFL.current = Green;
    ...
    std::cout<<int(Cabinettemp)<<"\n";
        int *CabinetThres = returnPolicy(int(Cabinettemp));
    ...
    std::cout<<int(LM75temp)<<"\n";
        int *LM75thres = returnPolicy(int(LM75temp));
        if(val < LM75thres[0]){
            foflManager.lm75TempFOFL.current = Green;
